...
const ProfileDetails: FC = () => {
  ...
  const logMessage = async (level:string, message:string) => {
    ...
    try {
      ...
      console.log('error','An error log')
      console.log('warn','A warn log');
      console.log('debug','A debug log');
        await axios.post('http://localhost:5002/api/logger1',({level,message}));
...
//     console.log(`${level} log message sent successfully`);
//   } catch (error) {
//     console.error(`Error sending ${level} log message:`, error);
//   }
// }
  ...
  const formik = useFormik<IProfileDetails>({
    ...
    onSubmit: async (values) => {
      ...
      // catch (error) {
      //            console.error('Error sending log message', error);
      //           alert('Failed to send log message chandu');
      //       }
      ...
      // await logMessage('info', `Form data submitted successfully: ${JSON.stringify(formData)}`);
    
    
      await logMessage('info', `Form data submitted successfully: ${JSON.stringify(values)}`);
      await logMessage('error','An error log');
      await logMessage('warn','A warn log');
      await logMessage('debug','A debug log');
      setLoading(true)
      setTimeout(() => {
